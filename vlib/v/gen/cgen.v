...
fn (mut g Gen) expr(node ast.Expr) {
	...
	match node {
		...
		ast.CallExpr {
			...
			// if g.pref.autofree && node.autofree_pregen.len != 0 { // g.strs_to_free0.len != 0 {
			/*
			if g.pref.autofree {
		...
		ast.SizeOf {
			if node.is_type {
				...
				if node.type_name.len == 0 || node.typ.has_flag(.generic) {
					styp = g.typ(node_typ)
				} else {
...
fn (mut g Gen) write_init_function() {
	...
	if g.pref.autofree {
		...
		// mb_size := if s_str_buf_size.len == 0 { 1 } else { s_str_buf_size.int() }
